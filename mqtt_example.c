...
#define EXAMPLE_MQTT_USER "rjct1997"
#define EXAMPLE_MQTT_PSWD "aio_enWd30ZrTZRnmENqzQQaJ33Suqzp"
...
#define MQTT_HUMIDITY_EVT		( 1 << 1 )
#define MQTT_SPRINKLERS_EVT		( 1 << 2 )
#define MQTT_LIGHT_EVT			( 1 << 3 )
#define MQTT_AIRQA_EVT			( 1 << 4 )
#define MQTT_DOORBELL_EVT		( 1 << 5 )
...
static int32_t get_simulated_sensor(int32_t current_value, int32_t max_step,
							 ...
							 bool increase);
static void humidity_timer_callback(TimerHandle_t pxTimer);
static void light_timer_callback(TimerHandle_t pxTimer);
static void airqa_timer_callback(TimerHandle_t pxTimer);
static void publish_humidity(void *ctx);
static void publish_light(void *ctx);
static void publish_gas(void *ctx);
...
TimerHandle_t xTimerHumidity, xTimerLight, xTimerAirQA;

uint32_t humidity_sensor = 50;
uint32_t samples_cnt = 0;
bool sprinklers_on, doorbell_ring;
uint32_t light_sensor = 50;
uint32_t gas_sensor = 5;
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    if(!memcmp(data, "On", 2)) {
    	...
    	doorbell_ring = true;
    	xEventGroupSetBits(xEventGroup,	MQTT_DOORBELL_EVT);
    }
...
{
    static const char *topics[] = {"rjct1997/feeds/sprinkler","rjct1997/feeds/guest"};
    int qos[]                   = {1,1};
    err_t err;
    int i;
...
static void publish_humidity(void *ctx)
{
    static const char *topic   = "rjct1997/feeds/humidity";
...
static void publish_light(void *ctx)
{
    static const char *topic   = "rjct1997/feeds/luminosity";
    static char message[10];

    LWIP_UNUSED_ARG(ctx);

    memset(message, 0, 10);
    sprintf(message, "%d", light_sensor);
...
static void publish_gas(void *ctx)
{
    static const char *topic   = "rjct1997/feeds/airquality";
    static char message[10];

    LWIP_UNUSED_ARG(ctx);

    memset(message, 0, 10);
    sprintf(message, "%d", gas_sensor);
...
static void app_thread(void *arg)
{
    ...
    xTimerHumidity = xTimerCreate("TimerHum", 1000 / portTICK_PERIOD_MS, pdTRUE, (void*)&timerId, humidity_timer_callback);
    xTimerLight = xTimerCreate("TimerLig", 3000 / portTICK_PERIOD_MS, pdTRUE, (void*)&timerId, light_timer_callback);
    xTimerAirQA = xTimerCreate("TimerGas", 5000 / portTICK_PERIOD_MS, pdTRUE, (void*)&timerId, airqa_timer_callback);
    if( xTimerHumidity == xTimerLight == xTimerAirQA == NULL ) {
    ...
    while(1) {
		...
		uxBits = xEventGroupWaitBits(
					...
					MQTT_CONNECTED_EVT | MQTT_HUMIDITY_EVT | MQTT_SPRINKLERS_EVT | MQTT_LIGHT_EVT | MQTT_AIRQA_EVT | MQTT_DOORBELL_EVT | MQTT_DISCONNECTED_EVT,	// The bits within the event group to wait for.
		...
		if(uxBits & MQTT_CONNECTED_EVT ) {
			...
			xTimerStart(xTimerHumidity, 0);
			xTimerStart(xTimerLight, 0);
			xTimerStart(xTimerAirQA, 0);
		}
		else if(uxBits & MQTT_HUMIDITY_EVT ) {
			PRINTF("MQTT_HUMIDTY_EVT.\r\n");
		...
		else if(uxBits & MQTT_LIGHT_EVT ) {
			PRINTF("MQTT_LIGHT_EVT.\r\n");
			// Simulate the light %, in steps of 5, range is 10% to 100%.
			light_sensor = get_simulated_sensor(light_sensor, 5, 10, 100, true);
			if((samples_cnt++%10) == 9){
				err = tcpip_callback(publish_light, NULL);
				if (err != ERR_OK)
				{
					PRINTF("Failed to invoke publish_light on the tcpip_thread: %d.\r\n", err);
				}
			}
		}
		else if(uxBits & MQTT_AIRQA_EVT ) {
			PRINTF("MQTT_AIRQA_EVT.\r\n");
			// Simulate the humidity %, in steps of 5, range is 10% to 100%.
			// If the sprinkler is On, the humidity will tent to rise.
			gas_sensor = get_simulated_sensor(gas_sensor, 1, 0, 10, true);
			if((samples_cnt++%10) == 9){
				err = tcpip_callback(publish_gas, NULL);
				if (err != ERR_OK)
				{
					PRINTF("Failed to invoke publish_gas on the tcpip_thread: %d.\r\n", err);
				}
			}
		}
		else if(uxBits & MQTT_DOORBELL_EVT ) {
			PRINTF("MQTT_DOORBELL_EVT.\r\n");
			if(doorbell_ring){
				GPIO_PortClear(BOARD_LED_GPIO, 1u << BOARD_LED_GPIO_PIN);
			}
			else {
...
void humidity_timer_callback( TimerHandle_t pxTimer )
{
	xEventGroupSetBits(xEventGroup,	MQTT_HUMIDITY_EVT);
}

void light_timer_callback( TimerHandle_t pxTimer )
{
	xEventGroupSetBits(xEventGroup,	MQTT_LIGHT_EVT);
}

void airqa_timer_callback( TimerHandle_t pxTimer )
{
	xEventGroupSetBits(xEventGroup,	MQTT_AIRQA_EVT);
}
